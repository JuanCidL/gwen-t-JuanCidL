...
class CloseBondTest extends FunSuite{
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    /** The 3rd for each card have the close bond effect
     * and the 2nd card have the same name of the 3rd card
     * */
    m1 = new MeleeCard("M1", "Melee example", 1)
    m2 = new MeleeCard("M3", "Melee example", 2)
    m3 = new MeleeCard("M3", "Melee example", 3)
    m3.setEffect(new CloseBond())
    r1 = new RangedCard("R1", "Ranged example", 1)
    r2 = new RangedCard("R3", "Ranged example", 2)
    r3 = new RangedCard("R3", "Ranged example", 3)
    r3.setEffect(new CloseBond())
    s1 = new SiegeCard("S1", "Siege example", 1)
    s2 = new SiegeCard("S3", "Siege example", 2)
    s3 = new SiegeCard("S3", "Siege example", 3)
    s3.setEffect(new CloseBond())
  ...
  test("Melee close bond"){
    ...
    assertEquals(m1.currentStrength, 1)
    assertEquals(m2.currentStrength, 2*2)
    assertEquals(m3.currentStrength, 3*2)
  ...
  test("Ranged close bond"){
    ...
    /** Check if the cards on the board had been affected */
    assertEquals(r1.currentStrength, 1)
    assertEquals(r2.currentStrength, 2*2)
    assertEquals(r3.currentStrength, 3*2)
    /** Check if the other type of cards had not affected */
    assertEquals(m1.currentStrength, 1)
    assertEquals(s1.currentStrength, 1)
    assertEquals(m2.currentStrength, 2)
  ...
  test("Siege close bond"){
    ...
    /** Check if the cards on the board had been affected */
    assertEquals(s1.currentStrength, 1)
    assertEquals(s2.currentStrength, 2*2)
    assertEquals(s3.currentStrength, 3*2)
    /** Check if the other type of cards had not affected */
    assertEquals(m1.currentStrength, 1)
    assertEquals(r1.currentStrength, 1)
    assertEquals(m2.currentStrength, 2)
