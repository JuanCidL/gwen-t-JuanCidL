...
class HoarfrostTest extends FunSuite{
  ...
  var hoarfrost1: WeatherCard = _
  var hoarfrost2: WeatherCard = _
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    hoarfrost1 = new WeatherCard("Hoar1", "Set strength = 1 to all melee cards on board")
    hoarfrost2 = new WeatherCard("Hoar2", "Set strength = 1 to all melee cards on board")
    hoarfrost1.setEffect(new Hoarfrost())
    hoarfrost2.setEffect(new Hoarfrost())
    ...
    deck = List()
    hand = List(m1, r1, s1, m2, r2, s2, clearWeather1, hoarfrost1)
    ...
    cpudeck = List()
    cpuhand = List(m3, r3, s3, m4, r4, s4, clearWeather2, hoarfrost2)

    cpu = new Player("CPU", 3, cpudeck, cpuhand)
    cpu.boardSection_(cpuzone)
  }

  test("Hoarfrost + ClearWeather by both players"){
    player.playCard(m1)
    cpu.playCard(m3)
    ...
    player.playCard(hoarfrost1)

    /** Check if the cards on the board had been affected */
    assertEquals(m1.currentStrength, 1)
    assertEquals(m2.currentStrength, 1)
    assertEquals(m3.currentStrength, 1)
    assertEquals(m4.currentStrength, 1)
