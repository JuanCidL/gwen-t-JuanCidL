...
class TorrentialRainTest extends FunSuite{
  ...
  var torrentialrain1: WeatherCard = _
  var torrentialrain2: WeatherCard = _
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    torrentialrain1 = new WeatherCard("Hoar1", "Set strength = 1 to all melee cards on board")
    torrentialrain2 = new WeatherCard("Hoar2", "Set strength = 1 to all melee cards on board")
    torrentialrain1.setEffect(new TorrentialRain())
    torrentialrain2.setEffect(new TorrentialRain())
    ...
    deck = List()
    hand = List(m1, r1, s1, m2, r2, s2, clearWeather1, torrentialrain1)
    ...
    cpudeck = List()
    cpuhand = List(m3, r3, s3, m4, r4, s4, clearWeather2, torrentialrain2)
  ...
  test("Impenetrable fog + ClearWeather by both players"){
    ...
    player.playCard(torrentialrain1)

    /** Check if the cards on the board had been affected */
    assertEquals(s1.currentStrength, 1)
    assertEquals(s2.currentStrength, 1)
    assertEquals(s3.currentStrength, 1)
    assertEquals(s4.currentStrength, 1)
